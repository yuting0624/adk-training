...
## S#29+ 本日組み立てる 3 つのエージェント (一覧)

**話すポイント**: Step 05 で Workflow に合成する前に「3 体それぞれが何屋なのか」を頭に入れてもらう。役割・ツール・入出力フォーマットを 1 ページに圧縮。

| | **Intake Agent** 📝 | **Triage Agent** 🔧 | **Recommend Agent** 🌐 |
|---|---|---|---|
| **役割** | 患者の自由記述から<br/>主訴とエリアを抽出 | 主訴から推奨診療科と<br/>緊急度を判定 | 診療科 + エリアから<br/>近隣の医療機関を検索 |
| **Model** | gemini-3.5-flash | gemini-3.5-flash | gemini-3.5-flash |
| **Tools** | なし<br/>(instruction のみ) | `lookup_specialty`<br/>(関数ツール) | Maps Grounding Lite<br/>(MCP) |
| **入力例** | 「最近よく胸が痛む。<br/>東京駅周辺で病院を」 | 主訴: 胸痛;<br/>エリア: 東京駅周辺 | 診療科: 循環器内科;<br/>緊急度: 高;<br/>エリア: 東京駅周辺 |
| **出力例** | 主訴: 胸痛;<br/>エリア: 東京駅周辺 | 診療科: 循環器内科;<br/>緊急度: 高;<br/>エリア: 東京駅周辺 | - 推奨診療科: 循環器内科<br/>- 緊急度: 高<br/>- 近隣の候補医療機関:<br/>&nbsp;&nbsp;1. ○○病院 (住所)<br/>&nbsp;&nbsp;2. ... |
| **設計ポイント** | 出力フォーマットを<br/>厳格に固定 (1 行) | ツールを **必ず呼ぶ**<br/>と instruction で強制 | 既存応答を **そのまま**<br/>保持しつつ末尾追記 |

> **キーメッセージ**:
> - 3 体とも同じ `gemini-3.5-flash`、違うのは **「役割」「ツール」「入出力契約」** だけ
> - 各エージェントの出力が **次のエージェントの入力フォーマット** になる = テキスト bucket relay
> - Step 05 の Workflow はこの 3 体を `edges` で繋ぐだけ → 次ページ

### 1 行 cheat sheet (口頭用)

> 「**自由文 → 構造化キーワード (Intake) → 診療科判定 (Triage) → 病院検索結果 (Recommend)**」
>
> 各段で抽象度が上がるのではなく、**コンテキストを蓄積しながら受け渡す** イメージ。
