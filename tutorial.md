...
- 最終出力に「## 診療科について (参考情報)」セクションと **実在する URL** の出典が追加されている

#### (c) 検索が実行されたか必ず検証する (重要)

`google_search` は **モデル判断で呼ばれる内蔵ツール** で、instruction が弱いと skip されて知識のみで出典を捏造することがあります。Events タブで evidence_agent の model_call を開き、以下を確認:

| 状態 | 判定 | 対処 |
|---|---|---|
| `grounding_metadata` あり + 出典 URL がレスポンスにも実在 | ✅ 検索された | OK |
| `grounding_metadata` 無し なのに出典が書かれている | ❌ **捏造** | 下記の対処を順に試す |
| `grounding_metadata` 無し + 「参考情報を取得できませんでした」 | ⚠️ instruction 通り fallback | 検索クエリを変えて再試行 |

**捏造を検出したときの対処** (上から順に試す):

1. **モデルを上げる**: `model="gemini-3.5-flash"` → `"gemini-3.5-pro"`。pro の方が built-in tool を素直に呼ぶ傾向
2. **instruction をさらに強化**: 冒頭に「あなたは Google 検索結果を引用するだけのアシスタントです。検索を実行しないと回答できません。」を追加
3. **クエリを明示**: instruction の手順に `google_search(query="循環器内科 代表的な症状")` のように関数呼び出し風の表記を入れる
...
- **内蔵ツールの「呼ばれるかは model 判断」問題**: 上の (c) で見たとおり、built-in tools は Function Tool と違って **必ず呼ばれる保証がない**。本番で確実性が必要なら、`google_search` の代わりに Custom Search API などを Function Tool として書く方が deterministic
